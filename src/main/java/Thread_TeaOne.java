...
public class Thread_TeaOne {                        //创建模拟甲乙丙都是并行行为的类，同时开始制作茶水活动
    ...
    static Thread thread_Yi_Water_Tea;              //创建类全局线程变量，用于存放乙同学制作茶水全过程的线程

    static Thread thread_Bing_Water_Tea;            //创建类全局线程变量，用于存放丙同学制作茶水全过程的线程

    static long time_Start;                                    //用来储存各位同学同时制作茶水的开始时间
    ...
    public static void main(String[] args) {                //创建主线程，用来模拟甲同学泡茶行为和结果，实现甲同学四个线程间的逻辑关系
        ...
        yi_Thread();                                        //调用方法，创建乙同学制作茶水全过程的线程

        bing_Thread();                                      //调用方法，创建丙同学制作茶水全过程的线程

        time_Start = System.currentTimeMillis();            //记录各位同学同时制作茶水的开始时间

        thread_Jia_kettle_AddWater.start();                 //启动线程，让CPU给予时间片，获取被任务调度器调度的资格

        thread_Yi_Water_Tea.start();                        //启动线程，让CPU给予时间片，获取被任务调度器调度的资格

        thread_Bing_Water_Tea.start();                      //启动线程，让CPU给予时间片，获取被任务调度器调度的资格
        ...
        log.debug("甲同学表示自己泡的茶很好喝,赞......");         //甲同学制作茶水工作模拟已完毕
    ...
    public static void jia_Thread(){                                 //此方法用于创建囊括甲同学的（洗水壶，放水），
            ...
            thread_Jia_kettle_AddWater = new Thread(()->{            //创建线程，模拟甲同学的（洗水壶，放水）过程
                ...
                thread_Jia_Water = new Thread(()->{         //创建线程模拟甲同学烧开水的过程
    ...
    public static void yi_Thread(){                         //此方法用来创建囊括乙同学全部行为的线程


        thread_Yi_Water_Tea = new Thread(()->{              //创建线程，模拟乙同学制作茶水工作全过程
            ...
            log.debug("乙同学表示自己泡的茶还不错,赞......");    //乙同学的制作茶水工作模拟已完毕
    ...
    public static void bing_Thread(){                       //此方法用来创建囊括丙同学全部行为的线程


        thread_Bing_Water_Tea = new Thread(()->{            //创建线程，模拟丙同学的制作茶水工作全过程
            ...
            log.debug("丙同学表示自己泡的茶很很狠很好喝,赞赞赞......"); //丙同学制作茶水工作模拟已完毕
...
class Make_Work {                                  //创建一个制作茶水的工具类，存放制作茶水各项工作的耗时方法,为各位同学的线程服务

    public static void clean_Kettle(){             //烧水损耗时间

        try {
            Thread.sleep(10000);             //洗水壶耗时10秒钟
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void heat_Up_Water(){           //烧开水损耗时间

        try {
            Thread.sleep(30000);            //烧开水耗时30秒钟
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void clean_Teapot(){            //洗茶壶损耗时间

        try {
            Thread.sleep(5000);             //洗茶壶耗时5秒钟
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void clean_Teacup(){           //洗茶杯损耗时间

        try {
            Thread.sleep(10000);           //洗茶杯耗时10秒钟
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void take_Tea(){              //拿茶叶损耗时间

        try {
            Thread.sleep(5000);           //拿茶叶耗时5秒钟
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void make_Tea(){              //泡茶损耗时间

        try {
            Thread.sleep(20000);          //泡茶损耗20秒钟
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
    public static void add_Water(){             //加水损耗时间

        try {
            Thread.sleep(5000);          //加水损耗5秒钟
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
